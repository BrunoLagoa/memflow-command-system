...
cursor_install_from_source() {
  ...
  for src_file in "${source_dir}"/*.md; do
    ...
    render_cursor_command_with_shared "$src_file" "$dest_file" "$source_dir"
    generated_count=$((generated_count + 1))
  done
  shopt -u nullglob

  if [[ "$generated_count" -eq 0 ]]; then
    die "Nenhum comando encontrado em ${source_dir} para instalação Cursor."
  fi
