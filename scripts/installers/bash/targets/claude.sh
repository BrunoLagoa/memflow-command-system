...
render_claude_command_with_shared() {
  local src_file="$1"
  local dest_file="$2"
  local source_dir="$3"
  ...
  render_command_with_shared_injection \
    ...
    "$dest_file" \
    "$source_dir" \
    "target-adapter.claude.md" \
    "### Conteúdo injetado: _shared/target-adapter.claude.md"
}

claude_install_from_source() {
  ...
  for src_file in "${source_dir}"/*.md; do
    ...
    render_claude_command_with_shared "$src_file" "$dest_file" "$source_dir"
    generated_count=$((generated_count + 1))
  done
  shopt -u nullglob

  if [[ "$generated_count" -eq 0 ]]; then
    die "Nenhum comando encontrado em ${source_dir} para instalação Claude."
  fi
